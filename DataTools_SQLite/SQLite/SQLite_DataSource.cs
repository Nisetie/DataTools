...
{
    public sealed class SQLite_DataSource : DataSource
    {
        private SQLiteConnection _conn = new SQLiteConnection();
        private IEnumerable<(string dbName, string schemaName)> _attached;
        private StringBuilder _query = new StringBuilder();

        public SQLite_DataSource() : base() { }

        public sealed override void Initialize(IDataContext dataContext)
        {
            base.Initialize(dataContext);
            if (!(dataContext is SQLite_DataContext context)) return;
            _conn.ConnectionString = context.ConnectionString;
            _attached = context.AttachedSchemas;
        }

        private string _AttachSchemas()
        {
            var sb = new StringBuilder();
            foreach (var s in _attached)
                sb.AppendLine($"attach '{s.dbName}' as {s.schemaName};");
            return sb.ToString();
        }
        ...
        {
            _conn.Open();
            new SQLiteCommand(_AttachSchemas() + query, _conn).ExecuteNonQuery();
            _conn.Close();
        }
        ...
        {
            _conn.Open();
            var result = new SQLiteCommand(_AttachSchemas() + query, _conn).ExecuteScalar();
            _conn.Close();
        ...
        public IEnumerable<object[]> ExecuteWithResult(string query)
        {
            ...
            try
            {
                reader = new SQLiteCommand(_AttachSchemas() + query, _conn).ExecuteReader(System.Data.CommandBehavior.SequentialAccess);
                if (reader.HasRows)
                {
